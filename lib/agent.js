...
• AN ALERT TYPE IS A BEHAVIOUR, NOT A TOOL (§6). Several sensors see the same thing, so three rules follow. One: when two sensors raise the same behaviour it is ONE incident, not two. Two: work the playbook entry, not the console — it governs regardless of which tool fired, and where two entries could apply take the one whose sensor carries the most context, usually NDR or EDR over the firewall, because they can say which process and which account rather than only which address. Three: A SENSOR THAT SAW NOTHING IS NOT EVIDENCE THAT NOTHING HAPPENED, especially where that sensor is not deployed on the asset in question. Record that silence as a data gap, never as a clean result.
...
export function buildPrompt(alert, enrichment, correlation, lessons, triggers = []) {
  ...
  // Wave-level rules, included only where they apply. These outrank the individual entry above:
  // the playbook says so for §9.1 and §9.2, and they are the rules most often skipped under volume.
  const rules = [
    ...(pb ? waveRules[pb.wave] || [] : []),
    ...(pb && /NDR/.test(pb.sensors || "") ? ndrRules : [])
  ];
  if (rules.length) {
    p += block(
      "PLAYBOOK — RULES FOR THIS WAVE, WHICH OUTRANK THE ENTRY ABOVE",
      rules.map((r) => `${r.title}\n${r.body}`).join("\n\n")
    );
  }
